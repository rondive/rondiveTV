...
1. **rondiveTV 前端**：已集成到本项目中，无需额外部署
...
#### 与本项目 docker-compose 集成（可选）

如果你已经使用本仓库的 `docker-compose.yml` 部署主站，可以新增一个 `watch-room-server` 服务：

```yaml
watch-room-server:
  image: tgs9915/watch-room-server:latest
...
然后在管理后台的「观影室配置」里填写服务器地址（例如 `https://your-domain.com` 或 `http://your-host:8080`）和同样的 `AUTH_KEY`。
...
部署完观影室服务器后，需要在 rondiveTV 管理后台配置：

1. 登录 rondiveTV 管理后台（需要 owner 或 admin 权限）
2. 进入"观影室配置"标签页
3. 填写配置信息：
   - **启用观影室功能**：勾选此选项
...
- **认证密钥**：用于验证 rondiveTV 客户端身份，防止未授权访问
...
**重要提示**：如果多个 rondiveTV 站点使用同一个观影室服务器：
...
- rondiveTV 配置的服务器地址是否正确？
- rondiveTV 配置的 AUTH_KEY 是否与服务器一致？
- 服务器防火墙是否开放了相应端口？
- 浏览器控制台是否有 CORS 错误？
...
- 服务器重启 = 所有房间清空
- Vercel 重新部署 rondiveTV = 不影响服务器（数据保留）
- 观影室服务器重新部署 = 所有房间清空
...
6. **独立部署**：每个 rondiveTV 站点使用独立的服务器（避免房间混淆）
...
- rondiveTV 观影室前端代码遵循 rondiveTV 项目许可证
...
- **rondiveTV 集成问题**：在 rondiveTV 项目仓库提交 Issue
