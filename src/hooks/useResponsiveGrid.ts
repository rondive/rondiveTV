...
export const useResponsiveGrid = (
  ...
  useLayoutEffect(() => {
    ...
    const scheduleCalculate = (width?: number) => {
      pendingWidth = width;
    ...
    const setupObserver = () => {
      ...
      scheduleCalculate(element.offsetWidth);

      // 使用ResizeObserver监听尺寸变化
      const resizeObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
          scheduleCalculate(entry.contentRect.width);
        }
      });
      ...
      // 窗口resize处理
      const handleResize = () => scheduleCalculate(element.offsetWidth);
