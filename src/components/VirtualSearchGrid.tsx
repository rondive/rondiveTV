...
export const VirtualSearchGrid = React.forwardRef<VirtualSearchGridRef, VirtualSearchGridProps>(({
  ...
  const imagesToPreload = useMemo(() => {
    if (!displayItemCount || columnCount <= 0) return [];

    const totalRows = Math.ceil(displayItemCount / columnCount);
    const startRow = Math.max(0, Math.min(visibleRange.start, totalRows - 1));
    const stopRow = Math.max(0, Math.min(visibleRange.stop, totalRows - 1));
    const preloadStart = Math.max(0, startRow - PRELOAD_ROW_BUFFER);
    const preloadStop = Math.min(totalRows - 1, stopRow + PRELOAD_ROW_BUFFER);

    const urls: string[] = [];
    const pushRow = (rowIndex: number) => {
      for (let colIndex = 0; colIndex < columnCount; colIndex += 1) {
        const index = rowIndex * columnCount + colIndex;
        if (index >= displayItemCount) return;
    ...
    for (let row = preloadStart; row < startRow; row += 1) {
      pushRow(row);
    }
    for (let row = stopRow + 1; row <= preloadStop; row += 1) {
      pushRow(row);
    }

    return urls;
  }, [displayData, displayItemCount, columnCount, viewMode, visibleRange.start, visibleRange.stop]);

  const preloadCount = Math.min(24, Math.max(columnCount * 3, 8));
  const preloadBatch = Math.min(8, Math.max(4, columnCount));

  useImagePreload(imagesToPreload, displayItemCount > 0, {
    maxPreload: preloadCount,
    maxBatch: preloadBatch,
    fetchPriority: 'low',
    idleTimeoutMs: 200,
    transformUrl: processImageUrl,
  });
      ...
      {totalItemCount === 0 ? (
        ...
        <Grid
          ...
          onCellsRendered={(visibleCells) => {
            ...
            const { rowStartIndex, rowStopIndex } = visibleCells;
            if (typeof rowStartIndex === 'number' && typeof rowStopIndex === 'number') {
              setVisibleRange((prev) =>
                prev.start === rowStartIndex && prev.stop === rowStopIndex
                  ? prev
                  : { start: rowStartIndex, stop: rowStopIndex }
              );
            }
