...
import type { ChatMessage, Member, Room } from '@/types/watch-room.types';
...
export function WatchRoomProvider({ children }: WatchRoomProviderProps) {
  ...
  const [_userNameLoaded, setUserNameLoaded] = useState(false);

  // Docker 内部域名在浏览器不可用，按当前站点主机名回退
  const resolvedServerUrl = useMemo(() => {
    if (!config?.serverUrl) return '';
    if (typeof window === 'undefined') return config.serverUrl;

    try {
      const parsed = new URL(config.serverUrl);
      if (parsed.hostname === 'watch-room-server') {
