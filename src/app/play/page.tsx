...
function PlayPageClient() {
  ...
  const [_commentsError, setCommentsError] = useState<string | null>(null);
  ...
  const [customAdFilterCode, setCustomAdFilterCode] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('customAdFilterCode') || '';
    }
    return '';
  });
  const [customAdFilterVersion, setCustomAdFilterVersion] = useState<number>(
    () => {
      if (typeof window !== 'undefined') {
        const storedVersion = parseInt(
          localStorage.getItem('customAdFilterVersion') || '1',
          10,
        );
  ...
  const customAdFilterVersionRef = useRef(customAdFilterVersion);

  useEffect(() => {
    customAdFilterCodeRef.current = customAdFilterCode;
    customAdFilterVersionRef.current = customAdFilterVersion;
  }, [customAdFilterCode, customAdFilterVersion]);

  useEffect(() => {
    if (typeof window === 'undefined') return;

    const loadCustomAdFilter = async () => {
      try {
        const response = await fetch('/api/ad-filter');
        if (!response.ok) {
          console.warn('Failed to load custom ad filter:', response.status);
          return;
        }
        const data = await response.json();
        const version = Number.parseInt(String(data?.version ?? '1'), 10) || 1;
        const code = typeof data?.code === 'string' ? data.code : '';

        setCustomAdFilterCode(code);
        setCustomAdFilterVersion(version);
        localStorage.setItem('customAdFilterCode', code);
        localStorage.setItem('customAdFilterVersion', String(version));
      } catch (error) {
        console.warn('Failed to load custom ad filter:', error);
      }
    };
  ...
  // 集数相关
  const getEpisodeIndexFromParams = () => {
  ...
  const downloadPollTimerRef = useRef<NodeJS.Timeout | null>(null);
  const downloadRequestInFlightRef = useRef(false);
  const downloadLastNoticeRef = useRef(0);
  const downloadLastMessageRef = useRef('');
  ...
  useEffect(() => {
    return () => {
      if (downloadPollTimerRef.current) {
        clearTimeout(downloadPollTimerRef.current);
      }
    };
  }, []);
  ...
  type Anime4kMode = 'A' | 'B' | 'C' | 'AA' | 'BB' | 'CA';
  const [anime4kEnabled, setAnime4kEnabled] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      const enabled = localStorage.getItem('enable_anime4k') === 'true';
      return enabled && !!navigator.gpu;
    }
    return false;
  });
  const [anime4kMode, setAnime4kMode] = useState<Anime4kMode>(() => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem('anime4k_mode') as Anime4kMode | null;
  ...
  const [anime4kScale, setAnime4kScale] = useState<number>(() => {
    if (typeof window !== 'undefined') {
      const storedScale = parseFloat(
        localStorage.getItem('anime4k_scale') || '2',
      );
      if (Number.isFinite(storedScale) && storedScale > 0) return storedScale;
    }
    return 2;
  });
  const anime4kCanvasRef = useRef<HTMLCanvasElement | null>(null);
  const anime4kStopRef = useRef(false);
  const anime4kVideoFrameCallbackRef = useRef<
    ((callback: VideoFrameRequestCallback) => number) | null
  >(null);
  const anime4kInitInFlightRef = useRef<Promise<void> | null>(null);
  ...
  useEffect(() => {
    const indexFromParams = getEpisodeIndexFromParams();
    if (indexFromParams !== currentEpisodeIndexRef.current) {
      setCurrentEpisodeIndex(indexFromParams);
    }

    const timeParam = searchParams.get('t');
    const parsedTime = timeParam ? parseFloat(timeParam) : 0;
    if (Number.isFinite(parsedTime) && parsedTime > 0) {
      resumeTimeRef.current = parsedTime;
    }
  }, [searchParams]);
  ...
  const fullSpeedTest = async (
    ...
    for (let i = 0; i < sources.length; i += concurrency) {
      ...
      console.log(
        `测速批次 ${Math.floor(i / concurrency) + 1}/${Math.ceil(sources.length / concurrency)}: ${batch.length} 个源`,
      );
  ...
  const stopAnime4k = () => {
    ...
    if (video) {
      if (anime4kVideoFrameCallbackRef.current) {
        video.requestVideoFrameCallback = anime4kVideoFrameCallbackRef.current;
        anime4kVideoFrameCallbackRef.current = null;
      }
      video.style.opacity = '';
  ...
  const initAnime4k = async () => {
    if (!webgpuSupported || !anime4kEnabled) return;
    const video = artPlayerRef.current?.video as HTMLVideoElement | undefined;
    const canvas = anime4kCanvasRef.current;
    if (!video || !canvas) return;

    if (anime4kInitInFlightRef.current) {
      await anime4kInitInFlightRef.current;
    }

    stopAnime4k();
    anime4kStopRef.current = false;
    ...
    const baseWidth = video.videoWidth || canvas.clientWidth || 1;
    const baseHeight = video.videoHeight || canvas.clientHeight || 1;
    const targetWidth = Math.max(1, Math.round(baseWidth * anime4kScale));
    const targetHeight = Math.max(1, Math.round(baseHeight * anime4kScale));
    ...
    if (typeof video.requestVideoFrameCallback === 'function') {
      ...
      anime4kVideoFrameCallbackRef.current = originalRequest;
      video.requestVideoFrameCallback = ((
        callback: VideoFrameRequestCallback,
      ) => {
        return originalRequest((now, metadata) => {
          if (anime4kStopRef.current) return;
          callback(now, metadata);
        });
      }) as typeof video.requestVideoFrameCallback;
    }

    const initPromise = (async () => {
      const anime4k = await import('anime4k-webgpu');
      const { render, ModeA, ModeB, ModeC, ModeAA, ModeBB, ModeCA } = anime4k;
      const modeMap: Record<Anime4kMode, any> = {
        A: ModeA,
        B: ModeB,
        C: ModeC,
        AA: ModeAA,
      ...
      const ModeClass = modeMap[anime4kMode] || ModeA;
    ...
    anime4kInitInFlightRef.current = initPromise;
    try {
      await initPromise;
    } catch (error) {
      console.error('Anime4K WebGPU init failed:', error);
      stopAnime4k();
    } finally {
      if (anime4kInitInFlightRef.current === initPromise) {
        anime4kInitInFlightRef.current = null;
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (!playerReady || !webgpuSupported) return;
  ...
  function filterAdsFromM3U8(m3u8Content: string): string {
    ...
    let contentToFilter = m3u8Content;

    const customAdFilterCodeText = customAdFilterCodeRef.current?.trim();
    if (customAdFilterCodeText) {
      try {
        const customFilter = new Function(
          'type',
          'm3u8Content',
          `${customAdFilterCodeText}\nreturn typeof filterAdsFromM3U8 === 'function' ? filterAdsFromM3U8(type, m3u8Content) : m3u8Content;`,
        ) as (type: string, m3u8Content: string) => string;
        const sourceType =
          currentSourceRef.current || detailRef.current?.source || '';
        const customResult = customFilter(sourceType, contentToFilter);
        if (typeof customResult === 'string' && customResult.length > 0) {
          contentToFilter = customResult;
        }
      } catch (error) {
        console.warn('Custom ad filter failed:', error);
      }
    }

    // 按行分割M3U8内容
    const lines = contentToFilter.split('\n');
    const filteredLines = [];
  ...
  useEffect(() => {
    ...
    const initFromHistory = async () => {
      ...
      const hasExplicitIndex = searchParams.get('index') !== null;
      const hasExplicitTime = searchParams.get('t') !== null;
      if (hasExplicitIndex || hasExplicitTime) {
        return;
      }
  ...
  const resetDownloadTracking = () => {
    ...
    downloadSignatureRef.current = null;
  };

  const notifyDownloadStatus = (message: string) => {
    if (!message) return;
    const now = Date.now();
    if (
      message === downloadLastMessageRef.current &&
      now - downloadLastNoticeRef.current < 3000
    ) {
      return;
    }
    downloadLastMessageRef.current = message;
    downloadLastNoticeRef.current = now;
    showPlayerNotice(message);
  };

  const formatDownloadStatusMessage = (data: any) => {
    ...
    if (status === 'running') {
      ...
      const speed = typeof progress.speed === 'string' ? progress.speed : '';
      if (percent !== null) {
        return `下载中 ${percent}%${speed ? ` (${speed})` : ''}`;
      }
      if (typeof progress.message === 'string' && progress.message) {
        if (progress.message === 'proxy') return '下载中（代理）';
        if (progress.message === 'direct') return '下载中（直连）';
      }
      return '正在下载...';
    }
    if (status === 'failed') {
  ...
  const triggerDownloadFile = (jobId: string) => {
    const downloadUrl = `/api/download/file?jobId=${encodeURIComponent(jobId)}&_t=${Date.now()}`;
    let iframe = downloadFrameRef.current;
    if (!iframe) {
  ...
  const pollDownloadStatus = async (jobId: string) => {
    try {
      const resp = await fetch(
        `/api/download/status?jobId=${encodeURIComponent(jobId)}&_t=${Date.now()}`,
      );
      if (!resp.ok) {
        const data = await resp.json().catch(() => ({}));
        const message = data?.error || '获取下载状态失败';
        notifyDownloadStatus(message);
        ...
        scheduleDownloadPoll(jobId, 4000);
        return;
      }

      const data = await resp.json().catch(() => ({}));
      if (!data || !data.status) {
        notifyDownloadStatus('下载状态未知');
        downloadInProgressRef.current = true;
        scheduleDownloadPoll(jobId, 4000);
        return;
      }
      ...
      if (data.status === 'completed') {
        resetDownloadTracking();
        clearDownloadPoll();
        notifyDownloadStatus('下载完成，正在准备传输');
        triggerDownloadFile(jobId);
        return;
      }
      ...
      if (data.status === 'failed') {
        resetDownloadTracking();
        clearDownloadPoll();
        notifyDownloadStatus(`下载失败：${data.error || '未知错误'}`);
        return;
      }

      downloadInProgressRef.current = true;
      notifyDownloadStatus(formatDownloadStatusMessage(data));
      scheduleDownloadPoll(jobId, data.status === 'queued' ? 2500 : 2000);
    } catch {
      notifyDownloadStatus('获取下载状态失败');
      scheduleDownloadPoll(jobId, 4000);
    }
  };

  const scheduleDownloadPoll = (jobId: string, delayMs = 2000) => {
    clearDownloadPoll();
    downloadPollTimerRef.current = setTimeout(() => {
      void pollDownloadStatus(jobId);
    }, delayMs);
  };

  const startDownloadPolling = (jobId: string) => {
    downloadJobIdRef.current = jobId;
    scheduleDownloadPoll(jobId, 800);
  };

  const getDownloadSignature = () => {
    const detailData = detailRef.current;
    const episodeIndex = currentEpisodeIndexRef.current;
    const resolvedTitle = videoTitleRef.current || detailData?.title || '';
    const episode =
      detailData?.episodes && detailData.episodes.length > 1
        ? String(episodeIndex + 1)
        : '';
    const episodeTitle = detailData?.episodes_titles?.[episodeIndex] || '';
  ...
  const resolveEpisodeDownloadUrl = async (episodeIndex: number) => {
    const detailData = detailRef.current;
    if (
      !detailData?.episodes ||
      episodeIndex < 0 ||
    ...
    if (episodeData.startsWith('shortdrama:')) {
      try {
        const [, videoId, episode] = episodeData.split(':');
        const nameParam = detailData.drama_name
          ? `&name=${encodeURIComponent(detailData.drama_name)}`
  ...
  const handleLocalDownload = async (episodeIndexes: number[]) => {
    const detailData = detailRef.current;
    if (!detailData?.episodes || detailData.episodes.length === 0) {
      showPlayerNotice('暂无可下载的播放地址');
      return;
    }
    if (!episodeIndexes.length) {
      showPlayerNotice('请先选择要下载的集数');
      return;
    }

    const format =
      typeof window !== 'undefined' &&
      localStorage.getItem('downloadFormat') === 'MP4'
        ? 'MP4'
        : 'TS';
    const baseTitle = videoTitleRef.current || detailData.title || '视频';
    let createdCount = 0;

    for (const index of episodeIndexes) {
      const url = await resolveEpisodeDownloadUrl(index);
      if (!url) continue;
      const episodeTitle =
        detailData.episodes_titles?.[index] || `第${index + 1}集`;
      try {
        await createTask(url, `${baseTitle} - ${episodeTitle}`, format);
        createdCount += 1;
      } catch (error) {
        console.error('创建本地下载任务失败:', error);
      }
    }

    if (createdCount > 0) {
      showPlayerNotice(`已创建 ${createdCount} 个本地下载任务`);
      setShowDownloadPanel(true);
    } else {
      showPlayerNotice('未能创建本地下载任务');
    }
  };

  const handleDownload = async () => {
    if (downloadRequestInFlightRef.current) {
      return;
    }

    if (!videoUrl) {
      showPlayerNotice('未获取到播放地址，无法服务端下载');
      return;
    }

    const downloadSignature = getDownloadSignature();
    const existingJobId = downloadJobIdRef.current;
    if (
      downloadInProgressRef.current &&
      existingJobId &&
      downloadSignatureRef.current === downloadSignature
    ) {
      notifyDownloadStatus('该任务正在下载，正在刷新进度');
      downloadSignatureRef.current = downloadSignature;
      startDownloadPolling(existingJobId);
      return;
    }
    ...
    if (downloadInProgressRef.current) {
      clearDownloadPoll();
      resetDownloadTracking();
      notifyDownloadStatus('已切换到新的下载任务');
    }

    downloadRequestInFlightRef.current = true;
    try {
      const params = new URLSearchParams();
      params.set('url', videoUrl);

      const resolvedTitle =
        videoTitleRef.current || detailRef.current?.title || '';
      if (resolvedTitle) {
      ...
      if (detailData?.episodes && detailData.episodes.length > 1) {
        params.set('episode', String(episodeIndex + 1));
        const episodeTitle = detailData.episodes_titles?.[episodeIndex];
        if (episodeTitle) {
      ...
      const checkResp = await fetch(
        `/api/download?check=1&${params.toString()}`,
      );
      if (!checkResp.ok) {
        const data = await checkResp.json().catch(() => ({}));
        const message = data?.error || '下载检查失败';
        showPlayerNotice(message);
        resetDownloadTracking();
        return;
      }
      ...
      if (checkData?.limitEnabled && typeof checkData.remaining === 'number') {
        notifyDownloadStatus(`今日剩余下载次数 ${checkData.remaining} 次`);
      } else {
        notifyDownloadStatus('开始准备下载');
      }

      const startResp = await fetch(`/api/download/start?${params.toString()}`);
      if (!startResp.ok) {
        const data = await startResp.json().catch(() => ({}));
        const message = data?.error || '创建下载任务失败';
        notifyDownloadStatus(message);
        resetDownloadTracking();
        return;
      }
      ...
      if (startData.status === 'completed') {
        resetDownloadTracking();
        notifyDownloadStatus('下载已完成，正在准备传输');
        triggerDownloadFile(jobId);
        return;
      }
      ...
      if (startData.status === 'failed') {
        resetDownloadTracking();
        notifyDownloadStatus(`下载失败：${startData.error || '未知错误'}`);
        return;
      }

      downloadInProgressRef.current = true;
      if (startData.deduped) {
        notifyDownloadStatus('已有相同任务，继续下载');
      } else {
        notifyDownloadStatus('开始下载');
      }
      startDownloadPolling(jobId);
    } catch (error) {
      console.error('下载请求异常:', error);
      notifyDownloadStatus('下载请求失败');
      resetDownloadTracking();
    } finally {
      downloadRequestInFlightRef.current = false;
    }
  };
        ...
        artPlayerRef.current = new Artplayer({
          ...
          customType: {
            m3u8: function (video: HTMLVideoElement, url: string) {
              ...
              const desktopBufferConfig = getHlsBufferConfig();
              ...
              const hls = new Hls({
                ...
                maxBufferLength: isMobile
                  ...
                  : desktopBufferConfig.maxBufferLength, // 桌面默认30s
                ...
                maxBufferSize: isMobile
                  ...
                  : desktopBufferConfig.maxBufferSize, // 桌面: 60MB (官方默认)
                ...
                maxMaxBufferLength: isMobile
                  ? localIsIOS13
                    ? 60
                    : 120
                  : desktopBufferConfig.maxMaxBufferLength, // 最大缓冲长度限制
          ...
          settings: [
                    ...
                    selector: [
                      {
                        html: '12px',
                        value: 12,
                        default: savedFontSize === 12,
                      },
                      {
                        html: '16px',
                        value: 16,
                        default: savedFontSize === 16,
                      },
                      {
                        html: '20px',
                        value: 20,
                        default: savedFontSize === 20,
                      },
                      {
                        html: '25px',
                        value: 25,
                        default: savedFontSize === 25,
                      },
                      {
                        html: '30px',
                        value: 30,
                        default: savedFontSize === 30,
                      },
                      {
                        html: '36px',
                        value: 36,
                        default: savedFontSize === 36,
                      },
                    ],
                    ...
                    selector: [
                      {
                        html: '30%',
                        value: 0.3,
                        default: savedOpacity === 0.3,
                      },
                      {
                        html: '50%',
                        value: 0.5,
                        default: savedOpacity === 0.5,
                      },
                      {
                        html: '70%',
                        value: 0.7,
                        default: savedOpacity === 0.7,
                      },
                      {
                        html: '80%',
                        value: 0.8,
                        default: savedOpacity === 0.8,
                      },
                      {
                        html: '100%',
                        value: 1.0,
                        default: savedOpacity === 1.0,
                      },
                    ],
                    onSelect: function (item: any) {
                      localStorage.setItem(
            ...
            },
            {
              name: 'Anime4K',
              html: 'Anime4K',
              icon: '<text x="50%" y="50%" font-size="14" font-weight="bold" text-anchor="middle" dominant-baseline="middle" fill="#ffffff">4K</text>',
              tooltip: !webgpuSupported
                ? '当前浏览器不支持 WebGPU'
                : anime4kEnabled
                  ? '已开启'
                  : '已关闭',
              switch: anime4kEnabled,
              onSwitch: function (item: any) {
                if (!webgpuSupported) {
                  showPlayerNotice('当前浏览器不支持 WebGPU');
                  return item.switch;
                }

                const nextState = !item.switch;
                try {
                  localStorage.setItem('enable_anime4k', String(nextState));
                } catch (_) {
                  // ignore
                }
                setAnime4kEnabled(nextState);
            ...
            },
            {
              name: 'Anime4K模式',
              html: 'Anime4K模式',
              icon: '<text x="50%" y="50%" font-size="12" font-weight="bold" text-anchor="middle" dominant-baseline="middle" fill="#ffffff">M</text>',
              selector: [
                { html: '模式A', value: 'A', default: anime4kMode === 'A' },
                { html: '模式B', value: 'B', default: anime4kMode === 'B' },
                { html: '模式C', value: 'C', default: anime4kMode === 'C' },
                { html: '模式AA', value: 'AA', default: anime4kMode === 'AA' },
                { html: '模式BB', value: 'BB', default: anime4kMode === 'BB' },
                { html: '模式CA', value: 'CA', default: anime4kMode === 'CA' },
              ],
              onSelect: function (item: any) {
                try {
                  localStorage.setItem('anime4k_mode', String(item.value));
                } catch (_) {
                  // ignore
                }
                setAnime4kMode(item.value);
                return item.html;
              },
            },
            {
              name: 'Anime4K倍率',
              html: 'Anime4K倍率',
              icon: '<text x="50%" y="50%" font-size="12" font-weight="bold" text-anchor="middle" dominant-baseline="middle" fill="#ffffff">X</text>',
              selector: [
                { html: '1x', value: 1, default: anime4kScale === 1 },
                { html: '1.5x', value: 1.5, default: anime4kScale === 1.5 },
                { html: '2x', value: 2, default: anime4kScale === 2 },
                { html: '3x', value: 3, default: anime4kScale === 3 },
              ],
              onSelect: function (item: any) {
                const nextScale = Number(item.value) || 2;
                try {
                  localStorage.setItem('anime4k_scale', String(nextScale));
                } catch (_) {
                  // ignore
                }
                setAnime4kScale(nextScale);
                return item.html;
              },
            },
          ],
          ...
          controls: [
            ...
            {
              position: 'right',
              html: '<i class="art-icon flex"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 3v12m0 0l4-4m-4 4l-4-4M4 17v2a2 2 0 002 2h12a2 2 0 002-2v-2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></i>',
              tooltip: '服务端下载',
              click: function () {
                handleDownload();
              },
  ...
  return (
    <PageLayout activePath='/play'>
            ...
            >
              <div className='relative w-full h-[300px] lg:h-full'>
                <canvas
                  ...
                  className={`absolute inset-0 z-[1] h-full w-full pointer-events-none transition-opacity ${
                    anime4kEnabled ? 'opacity-100' : 'opacity-0'
                  }`}
                />
                <div
                  ref={artRef}
                  className={`relative z-[2] w-full h-full rounded-xl overflow-hidden shadow-lg ${
                    anime4kEnabled ? 'bg-transparent' : 'bg-black'
                  }`}
                ></div>
                ...
                {pendingOwnerChange && !isRoomOwner && (
                  <div className='absolute inset-0 z-[650] flex items-center justify-center bg-black/60 backdrop-blur-sm'>
                    <div className='w-[90%] max-w-md rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-2xl'>
                      ...
                      <div className='p-4 space-y-4'>
                        <MiniVideoCard
                          title={pendingOwnerChange.videoName}
                          year={pendingOwnerChange.videoYear}
                          episode={pendingOwnerChange.episode}
                          poster={pendingOwnerChange.poster}
                          totalEpisodes={pendingOwnerChange.totalEpisodes}
                        />
                        <div className='flex gap-2'>
                          ...
                          <button
                            ...
                            className='flex-1 rounded-lg bg-gray-100 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 transition-colors'
                          >
                            自由观看
                          </button>
                        </div>
              ...
              {isInRoom && (
                <div className='mt-2 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-indigo-200/60 bg-indigo-50/80 px-3 py-2 text-sm text-indigo-700 dark:border-indigo-700/40 dark:bg-indigo-900/20 dark:text-indigo-200'>
                  <div className='flex items-center gap-2'>
                    ...
                    {!isRoomOwner && !isSameVideoAsOwner() && (
                  ...
                  {!isRoomOwner && (
                    <button
                      ...
                      className='rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-indigo-700 hover:bg-white dark:bg-gray-800/70 dark:text-indigo-200 dark:hover:bg-gray-800 transition-colors'
                    >
                      {syncPaused ? '跟随房主' : '暂停同步'}
                    </button>
                  )}
                </div>
              )}
                ...
                <div className='flex flex-col md:flex-row md:items-center gap-3'>
                  ...
                  <div className='flex items-center justify-center md:justify-start gap-2 flex-wrap'>
                    ...
                    <button
                      ...
                      className='group relative flex-shrink-0 transition-all duration-300 hover:scale-105'
                    >
                      <div className='absolute inset-0 bg-gradient-to-r from-emerald-400 to-green-400 rounded-full opacity-0 group-hover:opacity-30 blur-xl transition-opacity duration-300'></div>
                      <div className='relative flex items-center gap-1.5 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white px-3 py-1.5 rounded-full text-sm font-medium shadow-lg hover:shadow-xl transition-all duration-300'>
                    ...
                    <button
                      ...
                      className='group relative flex-shrink-0 transition-all duration-300 hover:scale-105'
                    >
                      <div className='absolute inset-0 bg-gradient-to-r from-gray-300 to-slate-300 rounded-full opacity-0 group-hover:opacity-30 blur-xl transition-opacity duration-300'></div>
                      <div className='relative flex items-center gap-1.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-3 py-1.5 rounded-full text-sm font-medium shadow-md hover:shadow-lg transition-all duration-300'>
                        下载管理
                      </div>
                    </button>
                  </div>
                            ...
                            <div
                              ...
                              ref={(node) => {
                                if (node) {
                                  ...
                                  const touchStartHandler = (_event: Event) => {
      ...
      <DownloadEpisodeSelector
        ...
        videoTitle={videoTitle || detail?.title || ''}
