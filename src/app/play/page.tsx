...
function PlayPageClient() {
  ...
  const handleDownload = async () => {
    ...
    if (!videoUrl) {
      showPlayerNotice('未获取到播放地址，无法下载');
      return;
    }
    ...
    try {
      ...
      const resolvedTitle = videoTitleRef.current || detailRef.current?.title || '';
      if (resolvedTitle) {
      ...
      const checkResp = await fetch(`/api/download?check=1&${params.toString()}`);
      if (!checkResp.ok) {
