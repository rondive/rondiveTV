...
function buildEnhancedStats(userStats: any, userCreatedAt: number) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const enhancedStats = buildEnhancedStats(userStats, userCreatedAt);
    const firstLoginTime = enhancedStats.firstLoginTime || 0;

    console.log('注册天数计算:', {
      userCreatedAt,
      userCreatedAtDate: new Date(userCreatedAt),
      registrationDays: enhancedStats.registrationDays,
      firstLoginTime: firstLoginTime,
      firstLoginTimeDate: firstLoginTime ? new Date(firstLoginTime) : null,
      loginDays: enhancedStats.loginDays,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { watchTime, movieKey, contentKey, timestamp } = body;
    const statsKey = contentKey || movieKey;

    if (typeof watchTime !== 'number' || watchTime <= 0 || !statsKey || typeof timestamp !== 'number') {
      return NextResponse.json(
        { error: '参数错误：需要 watchTime, contentKey 或 movieKey, timestamp' },
        { status: 400 }
      );
    }

    await db.updateUserStats(authInfo.username, statsKey, watchTime, timestamp);

    const updatedStats = await db.getUserPlayStat(authInfo.username);
    const PROJECT_START_DATE = new Date('2025-09-14').getTime();
    const userEntry = config.UserConfig.Users.find(
      (u) => u.username === authInfo.username
    );
    const userCreatedAt = userEntry?.createdAt || PROJECT_START_DATE;
    const enhancedStats = buildEnhancedStats(updatedStats, userCreatedAt);
