  ...
  try {
    ...
    for (const user of allUsers) {
      try {
        ...
        let lastLoginTime = 0;
        let loginCount = 0;
        let userPlayStat: any = null;
        try {
          userPlayStat = await storage.getUserPlayStat(user.username);
        ...
        if (records.length === 0) {
          ...
          userStats.push({
            ...
            totalWatchTime: userPlayStat?.totalWatchTime || 0,
            totalPlays: 0,
            lastPlayTime: 0,
            recentRecords: [],
            avgWatchTime: 0,
        ...
        // 计算用户统计
        const userWatchTime = userPlayStat?.totalWatchTime || 0;
