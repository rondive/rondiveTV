  ...
  try {
    ...
    if (!['userGroup', 'batchUpdateUserGroups'].includes(action) && targetUsername) {
      ...
      if (
        ...
        !['changePassword', 'updateUserApis', 'updateDownloadLimit', 'updateUserGroups'].includes(action)
      ) {
    ...
    switch (action) {
      ...
      case 'updateDownloadLimit': {
        ...
        const { downloadLimitEnabled, downloadLimitPerDay } = body as {
        ...
        if (
          ...
          return NextResponse.json(
            { error: '仅站长可配置其他管理员的下载限额' },
            { status: 401 }
          );
        }

        if (downloadLimitEnabled !== undefined) {
          targetEntry.downloadLimitEnabled = !!downloadLimitEnabled;
        }

        if (downloadLimitPerDay !== undefined) {
          const parsed = Number(downloadLimitPerDay);
          ...
          targetEntry.downloadLimitPerDay = Math.floor(parsed);
        }
