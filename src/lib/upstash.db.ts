...
export class UpstashRedisStorage implements IStorage {
  ...
  async getUserPlayStat(userName: string): Promise<UserPlayStat> {
    try {
      ...
      const statsStore = await this.client.get<UserStatsStore>(this.userStatsKey(userName));
      ...
      // 计算观看影片总数（去重）
      const totalMoviesFromRecords = new Set(playRecords.map(r => `${r.title}_${r.source_name}_${r.year}`)).size;
      ...
      const firstWatchDate = statsStore?.firstWatchDate
        ?? Math.min(...playRecords.map(r => r.save_time || Date.now()));
    ...
    try {
      ...
      for (const username of allUsers) {
        ...
        Object.entries(records).forEach(([key, record]) => {
          ...
          stat.playCount += 1;
          stat.totalWatchTime += contentWatchTime[key] || 0;
  ...
  async updateUserStats(
    ...
    try {
      ...
      const storedStats = await this.client.get<UserStatsStore>(statsKey);
      const stats: UserStatsStore = storedStats || {
        totalWatchTime: 0,
        lastUpdateTime: 0,
      ...
      await this.client.set(statsKey, stats);
      await this.deleteCache('play_stats_summary');
    } catch (error) {
