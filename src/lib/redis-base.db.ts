...
interface UserStatsStore {
  ...
  contentWatchTime?: Record<string, number>;
}
...
export abstract class BaseRedisStorage implements IStorage {
  ...
  private async getUserStatsStore(userName: string): Promise<UserStatsStore | null> {
    const raw = await this.withRetry(() => this.client.get(this.userStatsKey(userName)));
    if (!raw) {
      return null;
    }
    try {
      return JSON.parse(raw) as UserStatsStore;
    } catch (error) {
      console.error(`Failed to parse user stats store for ${userName}:`, error);
      return null;
    }
  }
  ...
  async getUserPlayStat(userName: string): Promise<UserPlayStat> {
    try {
      ...
      const statsStore = await this.getUserStatsStore(userName);
      const storedTotalWatchTime = statsStore?.totalWatchTime ?? 0;
      const storedTotalMovies = statsStore?.contentWatchTime
        ? Object.keys(statsStore.contentWatchTime).length
        : 0;
      const storedFirstWatchDate = statsStore?.firstWatchDate ?? Date.now();
      const storedLastUpdateTime = statsStore?.lastUpdateTime ?? Date.now();
      ...
      if (records.length === 0) {
        ...
        return {
          username: userName,
          totalWatchTime: storedTotalWatchTime,
          totalPlays: 0,
          lastPlayTime: 0,
          recentRecords: [],
          avgWatchTime: 0,
          mostWatchedSource: '',
          // 新增字段
          totalMovies: storedTotalMovies,
          firstWatchDate: storedFirstWatchDate,
      ...
      // 计算统计数据
      const totalWatchTime = storedTotalWatchTime;
      const totalPlays = records.length;
      const lastPlayTime = Math.max(...records.map(r => r.save_time || 0));

      // 计算观看影片总数（去重）
      const totalMoviesFromRecords = new Set(records.map(r => `${r.title}_${r.source_name}_${r.year}`)).size;
      const totalMovies = storedTotalMovies > 0 ? storedTotalMovies : totalMoviesFromRecords;

      // 计算首次观看时间
      const firstWatchDate = statsStore?.firstWatchDate
        ?? Math.min(...records.map(r => r.save_time || Date.now()));
      const lastUpdateTime = statsStore?.lastUpdateTime ?? Date.now();
  ...
  async getContentStats(limit = 10): Promise<ContentStat[]> {
    try {
      ...
      for (const username of allUsers) {
        const statsStore = await this.getUserStatsStore(username);
        const contentWatchTime = statsStore?.contentWatchTime || {};
        const playRecords = await this.getAllPlayRecords(username);
        ...
        Object.entries(playRecords).forEach(([key, record]) => {
          ...
          content.playCount++;
          content.totalWatchTime += contentWatchTime[contentKey] || 0;
  ...
  async updateUserStats(
    ...
    try {
      const statsKey = this.userStatsKey(userName);
      const raw = await this.withRetry(() => this.client.get(statsKey));
      const stats: UserStatsStore = raw
        ? (JSON.parse(raw) as UserStatsStore)
        : {
          totalWatchTime: 0,
          lastUpdateTime: 0,
      ...
      const contentWatchTime = stats.contentWatchTime || {};
      contentWatchTime[contentKey] = (contentWatchTime[contentKey] || 0) + watchTime;
      stats.contentWatchTime = contentWatchTime;
      stats.totalWatchTime = (stats.totalWatchTime || 0) + watchTime;
      stats.lastUpdateTime = timestamp;
      if (!stats.firstWatchDate || stats.firstWatchDate <= 0) {
        stats.firstWatchDate = timestamp;
      }

      await this.withRetry(() => this.client.set(statsKey, JSON.stringify(stats)));
      await this.deleteCache('play_stats_summary');
    } catch (error) {
